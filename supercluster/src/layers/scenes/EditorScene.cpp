...
namespace sc
{
	...
	void EditorScene::component_ui(bool enabled, std::uint64_t entity)
	{
		...
		if (batchedsprite != nullptr)
		{
			ImGui::Spacing();
			ImGui::Separator();
			ImGui::Text("Batched Sprite Component");

			ImGui::Spacing();
			ImGui::Spacing();

			auto region       = batchedsprite->get_region();
			...
			{
				batchedsprite->update_region(m_batch_region[0], m_batch_region[1], m_batch_region[2], m_batch_region[3]);
			}
		...
		if (point != nullptr)
		{
			ImGui::Spacing();
			ImGui::Separator();
			ImGui::Text("Point Component");

			ImGui::Spacing();
			ImGui::Spacing();

			int size = point->get_size();
			if (ImGui::SliderInt("Size", &size, 0, 100, "%d", ImGuiSliderFlags_AlwaysClamp))
			{
				point->set_size(size);
			}
		}
		...
		{
			if (ImGui::InputInt("Z-Level", &renderable->m_z_level, 1, 2, ImGuiInputTextFlags_AutoSelectAll | ImGuiInputTextFlags_EnterReturnsTrue))
			{
			}
		}
		...
		{
			/*
			glm::vec2 pos = batchedsprite->get_pos();
			if (ImGui::InputScalarN("Pos", ImGuiDataType_Float, &pos, 2))
			{
				batchedsprite->set_pos(pos.x, pos.y);
			}

			float rotation = batchedsprite->get_rotation();
			if (ImGui::SliderAngle("Rotate", &rotation))
			{
				batchedsprite->rotate(rotation);
			}

			float scale = batchedsprite->get_scale();
			if (ImGui::SliderFloat("Scale", &scale, 1, 10))
			{
				batchedsprite->scale(scale);
			}
			*/
		}
	}
