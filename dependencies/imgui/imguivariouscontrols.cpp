...
#include "imgui/addons/imguivariouscontrols.h"
...
namespace ImGui
{
	...
	static bool CheckButton(const char* label, bool* pvalue, bool useSmallButton, float checkedStateAlphaMult = 0.5f)
	{
		...
		if (tmp)
		{
			...
			ImVec4 CheckButtonColor = style.Colors[ImGuiCol_Button];
			CheckButtonColor.w *= checkedStateAlphaMult;
			ImVec4 CheckButtonHoveredColor = style.Colors[ImGuiCol_ButtonHovered];
			CheckButtonHoveredColor.w *= checkedStateAlphaMult;
			ImVec4 CheckButtonActiveColor = style.Colors[ImGuiCol_ButtonActive];
	...
	{
		return CheckButton(label, pvalue, false);
	}
	bool SmallCheckButton(const char* label, bool* pvalue)
	{
		return CheckButton(label, pvalue, true);
	}
	...
	float ProgressBar(const char* optionalPrefixText, float value, const float minValue, const float maxValue, const char* format, const ImVec2& sizeOfBarWithoutTextInPixels, const ImVec4& colorLeft, const ImVec4& colorRight, const ImVec4& colorBorder)
	{
		...
		if (size.x <= 0)
			size.x = ImGui::GetWindowWidth() * 0.25f;
		if (size.y <= 0)
			size.y = ImGui::GetTextLineHeightWithSpacing(); // or without
	...
	{
		int selectedEntry = -1;
		if (pOptionalHoveredEntryOut)
			*pOptionalHoveredEntryOut = -1;
		if (!open)
		...
		if (ImGui::BeginPopup("MyOwnPopupSimpleMenu"))
		{
			...
			if (scrollUpEntryText && needsScrolling)
			{
				...
				if (reverseItems ? (endIndex + 1 < numEntries) : (startIndex > 0))
				{
					...
					else if (pOptionalHoveredEntryOut && ImGui::IsItemHovered())
						*pOptionalHoveredEntryOut = entryIndex;
				}
				else
					ImGui::Text(" ");
	...
	int PopupMenuSimpleCopyCutPasteOnLastItem(bool readOnly)
	{
		...
		if (selectedEntry > 2)
			selectedEntry = 2; // Normally separator takes out one space
		return selectedEntry;
	...
	{
		if (numAllowedEntries < 1 || numTotalEntries == 0)
		{
			params.open = false;
			return -1;
		}
		if (params.endIndex == -1)
		...
		{
			if (oldHoveredEntry != params.hoveredEntry)
				params.scrollTimer = ImGui::GetTime();
			...
			if (params.selectedEntry <= -2 || (newTime - params.scrollTimer > 0.4f))
			{
				...
				else if (params.hoveredEntry == -3 || params.selectedEntry == -3)
				{
					if (params.endIndex < numTotalEntries - 1)
					{
						++params.startIndex;
						++params.endIndex;
					}
	...
	inline static bool ColorChooserInternal(ImVec4* pColorOut, bool supportsAlpha, bool showSliders, ImGuiWindowFlags extra_flags = 0, bool* pisAnyItemActive = NULL, float windowWidth = 180 /*,bool isCombo = false*/)
	{
		...
		//ImGui::BeginGroup();
		{
			...
			for (int y = 0; y < step; y++)
			{
				for (int x = 0; x < step; x++)
				{
					...
					window->DrawList->AddRectFilledMultiColor(window->Pos + pos, window->Pos + pos + ImVec2(quadSize / step, quadSize / step), ImGui::ColorConvertFloat4ToU32(c00), ImGui::ColorConvertFloat4ToU32(c10), ImGui::ColorConvertFloat4ToU32(c11), ImGui::ColorConvertFloat4ToU32(c01));
		...
		//ImGui::BeginGroup();
		{
			...
			for (int y = 0; y < step; y++)
			{
				...
				window->DrawList->AddRectFilledMultiColor(window->Pos + pos, window->Pos + pos + ImVec2(width, quadSize / step), ColorConvertFloat4ToU32(c0), ColorConvertFloat4ToU32(c0), ColorConvertFloat4ToU32(c1), ColorConvertFloat4ToU32(c1));
		...
		if (showSliders)
		{
			...
			ImGui::SameLine();
			if (ImGui::SmallButton(btnNames[index]))
				useHsvSliders = !useHsvSliders;
			...
			{
				{
					...
					if (sliderMoved)
					{
						...
						if (supportsAlpha)
							color.w = (float)a / 255.f;
					}
					//ColorConvertRGBtoHSV(s_color.x, s_color.y, s_color.z, tint, sat, val);*/
					if (pisAnyItemActive)
						*pisAnyItemActive |= sliderMoved;
				}
			}
			ImGui::EndChild();
		}

		if (colorSelected && pColorOut)
			*pColorOut = color;
	...
	bool ColorCombo(const char* label, ImVec4* pColorOut, bool supportsAlpha, float width, bool closeWhenMouseLeavesIt)
	{
		...
		if (totalSize.x < itemWidth)
			totalSize.x = itemWidth;
		...
		RenderFrame(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y + 1), ImVec2(frame_bb.Min.x + color_quad_size - 1, frame_bb.Max.y - 1), ImGui::ColorConvertFloat4ToU32(ImVec4(color.x, color.y, color.z, 1.f)), true, style.FrameRounding);
		...
		if (hovered)
		{
			...
			else if (g.IO.MouseClicked[2] && pCopiedColor && pColorOut)
			{ // middle-click (paste color)
				pColorOut->x = pCopiedColor->x;
				pColorOut->y = pCopiedColor->y;
				pColorOut->z = pCopiedColor->z;
				if (supportsAlpha)
					pColorOut->w = pCopiedColor->w;
				color = *pColorOut;
	...
	static int MultilineScrollCallback(ImGuiInputTextCallbackData* data)
	{
		...
		if (oldCursorIndex != data->CursorPos)
		{
			...
			if (cursorOffset < scrollState->scrollX)
			{
				...
				scrollState->newScrollX                 = cursorOffset - SCROLL_INCREMENT;
				if (scrollState->newScrollX < 0)
					scrollState->newScrollX = 0;
			}
		...
		{
			scrollState->SelectionStart = data->SelectionStart;
			scrollState->SelectionEnd   = data->SelectionEnd;
		}
		else
		{
			scrollState->SelectionStart = data->SelectionEnd;
			scrollState->SelectionEnd   = data->SelectionStart;
		}
	...
	bool InputTextMultilineWithHorizontalScrolling(const char* label, char* buf, size_t buf_size, float height, ImGuiInputTextFlags flags, bool* pOptionalIsHoveredOut, int* pOptionalCursorPosOut, int* pOptionalSelectionStartOut, int* pOptionalSelectionEndOut, float SCROLL_WIDTH)
	{
		...
		scrollState.scrollRegionX = ImGui::GetWindowWidth() - scrollbarSize;
		if (scrollState.scrollRegionX < 0)
			scrollState.scrollRegionX = 0;
		scrollState.scrollX = ImGui::GetScrollX();
		scrollState.storage = ImGui::GetStateStorage();
		bool changed        = ImGui::InputTextMultiline(label, buf, buf_size, ImVec2(SCROLL_WIDTH - scrollbarSize, (height - scrollbarSize) > 0 ? (height - scrollbarSize) : 0), flags | ImGuiInputTextFlags_CallbackAlways, MultilineScrollCallback, &scrollState);
		if (pOptionalIsHoveredOut)
			*pOptionalIsHoveredOut = ImGui::IsItemHovered();
		...
		if (pOptionalCursorPosOut)
			*pOptionalCursorPosOut = scrollState.CursorPos;
		if (pOptionalSelectionStartOut)
			*pOptionalSelectionStartOut = scrollState.SelectionStart;
		if (pOptionalSelectionEndOut)
			*pOptionalSelectionEndOut = scrollState.SelectionEnd;
	...
	bool InputTextMultilineWithHorizontalScrollingAndCopyCutPasteMenu(const char* label, char* buf, int buf_size, float height, bool& staticBoolVar, int* staticArrayOfThreeIntegersHere, ImGuiInputTextFlags flags, bool* pOptionalHoveredOut, float SCROLL_WIDTH, const char* copyName, const char* cutName, const char* pasteName)
	{
		...
		if (pOptionalHoveredOut)
			*pOptionalHoveredOut = isHovered;
		// Popup Menu ------------------------------------------
		...
		if (hasSelectedText || !readOnly)
		{
			...
			else
			{
				...
				if (sel == 3)
					sel = 2; // Normally separator takes out one space
				...
				if (mustCopy || mustCut || (mustPaste && (selectionStart < selectionEnd)))
				{
					...
					if (!mustPaste)
					{
						const char tmp    = buf[selectionEnd];
						buf[selectionEnd] = '\0';
					...
					if (!mustCopy)
					{
						...
						for (int i = 0, isz = buf_size - selectionEnd; i < isz; i++)
							buf[i + selectionStart] = buf[i + selectionEnd]; // I do it manually, so Valgrind is happy

						for (int i = selectionStart + buf_size - selectionEnd; i < buf_size; i++)
							buf[i] = '\0'; // This is mandatory at the end
					}
					popup_open = false;
				}
				if (mustPaste)
				{
					...
					for (int i = cursorPosition, isz = cursorPosition + clipboardTextSize >= buf_size ? buf_size : cursorPosition + clipboardTextSize; i < isz; i++)
						buf[i] = clipboardText[i - cursorPosition];
	...
	protected:
		...
		inline void updateTexture() const
		{
			...
			{
				if (!texId)
					AnimatedImage::GenerateOrUpdateTextureCb(texId, w, h, 4, buffer, false, false, false);
				return;
			}

			// These two lines sync animation in multiple items:
			if (texId && lastImGuiFrameUpdate == ImGui::GetFrameCount())
				return;
			...
			for (int i = lastFrameNum; i < frames; i++)
			{
				...
				if (delay <= lastDelay + frameTime)
				{
					...
					{
						if (!persistentTexId)
							AnimatedImage::GenerateOrUpdateTextureCb(texId, w, h, 4, &buffer[imageSz * i], false, false, false);
						else
						{
		...
		struct ScopedFileContent
		{
			...
			static stbi_uc* GetFileContent(const char* filePath, int* size_out)
			{
				...
				*size_out          = 0;
				if (!filePath || (f = ImFileOpen(filePath, "rb")) == NULL)
					return NULL;
				if (fseek(f, 0, SEEK_END) || (f_size = ftell(f)) == -1 || fseek(f, 0, SEEK_SET))
				{
					fclose(f);
					return NULL;
				}
				...
				fclose(f);
				if (f_size_read == 0 || f_size_read != (size_t)f_size)
				{
					STBI_FREE(f_data);
					return NULL;
				}
			...
			ScopedFileContent(const char* filePath)
			{
				gif_buffer = GetFileContent(filePath, &gif_buffer_size);
			}
			~ScopedFileContent()
			{
				if (gif_buffer)
				{
					STBI_FREE(gif_buffer);
					gif_buffer = NULL;
				}
				gif_buffer_size = 0;
			}
	...
	public:
		AnimatedImageInternal()
		{
			buffer                    = NULL;
			persistentTexIdIsNotOwned = false;
			texId = persistentTexId = NULL;
			clear();
		}
		~AnimatedImageInternal()
		{
...
#ifndef STBI_NO_GIF
#ifndef IMGUIVARIOUSCONTROLS_NO_STDIO
		AnimatedImageInternal(char const* filename, bool useHoverModeIfSupported = false)
		{
			buffer                    = NULL;
			persistentTexIdIsNotOwned = false;
			texId = persistentTexId = NULL;
			load(filename, useHoverModeIfSupported);
		}
#endif //IMGUIVARIOUSCONTROLS_NO_STDIO
		AnimatedImageInternal(const unsigned char* memory_gif, int memory_gif_size, bool useHoverModeIfSupported = false)
		{
			buffer                    = NULL;
			persistentTexIdIsNotOwned = false;
			texId = persistentTexId = NULL;
			load_from_memory(memory_gif, memory_gif_size, useHoverModeIfSupported);
		}
#endif //STBI_NO_GIF
		...
		void clear()
		{
			...
			uvFrame0.x = uvFrame0.y = 0;
			uvFrame1.x = uvFrame1.y = 1;
			lastImGuiFrameUpdate    = -1;
			hoverModeIfSupported = isAtLeastOneWidgetInHoverMode = false;
			if (texId || persistentTexId)
				IM_ASSERT(AnimatedImage::FreeTextureCb != NULL); // Please use ImGui::AnimatedImage::SetFreeTextureCallback(...)
			if (texId)
			{
				if (texId != persistentTexId)
					AnimatedImage::FreeTextureCb(texId);
				texId = NULL;
			}
			if (persistentTexId)
			{
				if (!persistentTexIdIsNotOwned)
					AnimatedImage::FreeTextureCb(persistentTexId);
				persistentTexId = NULL;
			}
		}
#ifndef STBI_NO_GIF
#ifndef IMGUIVARIOUSCONTROLS_NO_STDIO
			...
			if (AnimatedImage::MaxPersistentTextureSize.x > 0 && AnimatedImage::MaxPersistentTextureSize.y > 0)
			{
				...
				if (maxNumFramesInATexture >= frames)
				{
					...
					if (hoverModeIfSupported)
						delays[0] = 0.f; // Otherwise when we start hovering, we usually get an unwanted delay
				}
			}
		...
		bool create(ImTextureID myTexId, int animationImageWidth, int animationImageHeight, int numFrames, int numFramesPerRowInTexture, int numFramesPerColumnInTexture, float delayDetweenFramesInCs, bool useHoverMode = false)
		{
			...
			if (!myTexId || animationImageWidth <= 0 || animationImageHeight <= 0 || numFrames <= 0 || delayDetweenFramesInCs <= 0 || (numFramesPerRowInTexture * numFramesPerColumnInTexture < numFrames))
				return false;
			...
			delays.resize(frames);
			for (int i = 0; i < frames; i++)
				delays[i] = delayDetweenFramesInCs;
			hoverModeIfSupported = useHoverMode;
			return true;
		}

		inline bool areAllFramesInASingleTexture() const
		{
			return persistentTexId != NULL;
		}
		void render(ImVec2 size = ImVec2(0, 0), const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), const ImVec4& tint_col = ImVec4(1, 1, 1, 1), const ImVec4& border_col = ImVec4(0, 0, 0, 0)) const
		{
			...
			if (persistentTexId)
			{
				...
				if (hoverModeIfSupported)
				{
					...
					if (hovered)
						isAtLeastOneWidgetInHoverMode = true;
				}
				if (hovered)
				{
		...
		bool renderAsButton(const char* label, ImVec2 size = ImVec2(0, 0), const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 = ImVec2(1, 1), int frame_padding = -1, const ImVec4& bg_col = ImVec4(0, 0, 0, 0), const ImVec4& tint_col = ImVec4(1, 1, 1, 1)) const
		{
			...
			ImVec2 uv_0 = uv0;
			ImVec2 uv_1 = uv1;
			if (hovered && hoverModeIfSupported)
				isAtLeastOneWidgetInHoverMode = true;
		...
		inline ImTextureID getTexture() const
		{
			return texId;
		}
...
#undef DEBUG_OUT_TEXTURE
	...
	AnimatedImage::AnimatedImage(const char* gif_filepath, bool useHoverModeIfSupported)
	{
		...
		IM_PLACEMENT_NEW(ptr)
		AnimatedImageInternal(gif_filepath, useHoverModeIfSupported);
	}
#endif //IMGUIVARIOUSCONTROLS_NO_STDIO
	AnimatedImage::AnimatedImage(const unsigned char* gif_buffer, int gif_buffer_size, bool useHoverModeIfSupported)
	{
		ptr = (AnimatedImageInternal*)ImGui::MemAlloc(sizeof(AnimatedImageInternal));
		IM_PLACEMENT_NEW(ptr)
		AnimatedImageInternal(gif_buffer, gif_buffer_size, useHoverModeIfSupported);
	}
#endif //STBI_NO_GIF
	AnimatedImage::AnimatedImage(ImTextureID myTexId, int animationImageWidth, int animationImageHeight, int numFrames, int numFramesPerRowInTexture, int numFramesPerColumnInTexture, float delayBetweenFramesInCs, bool useHoverMode)
	{
		ptr = (AnimatedImageInternal*)ImGui::MemAlloc(sizeof(AnimatedImageInternal));
		IM_PLACEMENT_NEW(ptr)
		AnimatedImageInternal(myTexId, animationImageWidth, animationImageHeight, numFrames, numFramesPerRowInTexture, numFramesPerColumnInTexture, delayBetweenFramesInCs, useHoverMode);
	}
	...
	void AnimatedImage::render(ImVec2 size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col) const
	{
		ptr->render(size, uv0, uv1, tint_col, border_col);
	}
	bool AnimatedImage::renderAsButton(const char* label, ImVec2 size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
	{
		return ptr->renderAsButton(label, size, uv0, uv1, frame_padding, bg_col, tint_col);
	}
#ifndef STBI_NO_GIF
#ifndef IMGUIVARIOUSCONTROLS_NO_STDIO
	bool AnimatedImage::load(const char* gif_filepath, bool useHoverModeIfSupported)
	{
		return ptr->load(gif_filepath, useHoverModeIfSupported);
	}
#endif //IMGUIVARIOUSCONTROLS_NO_STDIO
	bool AnimatedImage::load_from_memory(const unsigned char* gif_buffer, int gif_buffer_size, bool useHoverModeIfSupported)
	{
		return ptr->load_from_memory(gif_buffer, gif_buffer_size, useHoverModeIfSupported);
	}
#endif //STBI_NO_GIF
	bool AnimatedImage::create(ImTextureID myTexId, int animationImageWidth, int animationImageHeight, int numFrames, int numFramesPerRowInTexture, int numFramesPerColumnInTexture, float delayBetweenFramesInCs, bool useHoverMode)
	{
	...
	int AnimatedImage::getNumFrames() const
	{
		return ptr->getNumFrames();
	}
	bool AnimatedImage::areAllFramesInASingleTexture() const
	{
		return ptr->areAllFramesInASingleTexture();
	}
#endif //NO_IMGUIVARIOUSCONTROLS_ANIMATEDIMAGE
...
*/
	...
	static int AppendTreeNodeHeaderButtonsV(const void* ptr_id, float startWindowCursorXForClipping, int numButtons, va_list args)
	{
		if (!ImGui::IsItemVisible())
			return -1; // We don't reset non-togglable buttons to false for performance reasons
		...
		{
			pPressed       = va_arg(args, bool*);
			tooltip        = va_arg(args, const char*);
			glyph          = va_arg(args, const char*);
			isToggleButton = va_arg(args, int);
			...
			if (pPressed)
			{
				...
				if (pos.x > startWindowCursorXForClipping + (4.0f * buttonSz))
				{
					...
					if ((pressed = ImGui::GlyphButton(id, pos, glyphHalfSize, glyph, isToggleButton ? pPressed : NULL, &hovered)))
						rv = i;
					if (!isToggleButton)
						*pPressed = pressed;
					pos.x -= buttonSz - 2;
					if (tooltip && hovered && strlen(tooltip) > 0)
				...
				else
				{
					pressed = false;
					if (!isToggleButton)
						*pPressed = false; // Just in case we pass a static bool
				}
				//if (pressed) fprintf(stderr,"Pressed button %d: '%s'\n",i,glyph?glyph:"close"); // TO REMOVE
			}
			else
				pos.x -= buttonSz - 2; // separator mode
		}
		return rv;
	}
	...
	struct ImGuiPlotMultiArrayGetterData
	{
		const float** Values;
		int Stride;
		ImGuiPlotMultiArrayGetterData(const float** values, int stride)
		{
			Values = values;
			Stride = stride;
		}
		...
		{
			if (fillColorGradientDeltaIn0_05 == 0)
			{
				tc = bc = c;
				return;
			}
			...
			if (negative)
				fillColorGradientDeltaIn0_05 = -fillColorGradientDeltaIn0_05;
			if (fillColorGradientDeltaIn0_05 > 0.5f)
				fillColorGradientDeltaIn0_05 = 0.5f;
		...
		inline static bool IsMouseBetweenXValues(float x_min, float x_max, float* pOptionalXDeltaOut = NULL, bool clip = true, bool expandForTouchInput = false)
		{
			...
			if (clip)
			{
				if (x_min < window->ClipRect.Min.x)
					x_min = window->ClipRect.Min.x;
				if (x_max > window->ClipRect.Max.x)
					x_max = window->ClipRect.Max.x;
			}
			if (expandForTouchInput)
			{
				x_min -= g.Style.TouchExtraPadding.x;
				x_max += g.Style.TouchExtraPadding.x;
			}

			if (pOptionalXDeltaOut)
				*pOptionalXDeltaOut = g.IO.MousePos.x - x_min;
	...
	{
		ImGuiWindow* window = GetCurrentWindow();
		if (pOptionalHoveredHistogramIndexOut)
			*pOptionalHoveredHistogramIndexOut = -1;
		if (window->SkipItems)
			return -1;
		...
		if (scale_min == FLT_MAX || scale_max == FLT_MAX || scale_min == scale_max)
		{
			...
			if (scale_min == FLT_MAX || scale_min >= scale_max)
				scale_min = v_min;
			if (scale_max == FLT_MAX || scale_min >= scale_max)
				scale_max = v_max;
		}
		...
		if (values_count > 0 && num_histograms > 0)
		{
			...
			if (!mustOverrideColors)
				col_base_embedded[0] = GetColorU32(ImGuiCol_PlotHistogram);
			...
			if (t_step < minSingleHistogramWidth)
				t_step = minSingleHistogramWidth;
			else if (t_step > maxSingleHistogramWidth)
				t_step = maxSingleHistogramWidth;

			float t1   = 0.f;
			float posY = 0.f;
			ImVec2 pos0(0.f, 0.f), pos1(0.f, 0.f);
			ImU32 rectCol = 0, topRectCol = 0, bottomRectCol = 0;
			float gradient      = 0.f;
			short overflow      = 0;
			bool mustSkipBorder = false;
			int iWithOffset     = 0;
			for (int i = 0; i < values_count; i++)
			{
				if (i != 0)
					t1 += histogramGroupSpacingInPixels;
				if (t1 > inner_bb.Max.x)
					break;
				...
				for (int h = 0; h < num_histograms; h++)
				{
					    ...
					    ImGuiPlotMultiArrayGetterData::IsMouseBetweenXValues(pos0.x, pos1.x))
					{
						...
						if (pOptionalHoveredHistogramIndexOut)
							*pOptionalHoveredHistogramIndexOut = h_hovered;
						SetTooltip("%d: %8.4g", i, v1); // Tooltip on hover

						if (h == 0 && !mustOverrideColors)
							rectCol = GetColorU32(ImGuiCol_PlotHistogramHovered); // because: col_base[0] = GetColorU32(ImGuiCol_PlotHistogram);
						else
						{
							...
							if (style.Alpha != 0.f)
								base.w /= style.Alpha; // See GetColorU32(...) for this
							base.x += diff.x;
							base.y += diff.y;
							base.z += diff.z;
							base.w += diff.w;
					...
					if (overflow != 0)
					{
						...
						if (overflow > 0)
							window->DrawList->AddTriangleFilled(ImVec2((pos0.x + pos1.x) * 0.5f, inner_bb.Min.y + spacing), ImVec2(pos1.x - spacing, inner_bb.Min.y + spacing + height), ImVec2(pos0.x + spacing, inner_bb.Min.y + spacing + height), overflowCol);
						else if (overflow < 0)
							window->DrawList->AddTriangleFilled(ImVec2((pos0.x + pos1.x) * 0.5f, inner_bb.Max.y - spacing), ImVec2(pos0.x + spacing, inner_bb.Max.y - spacing - height), ImVec2(pos1.x - spacing, inner_bb.Max.y - spacing - height), overflowCol);
					}
	...
	{
		ImGuiWindow* window = GetCurrentWindow();
		if (pOptionalHoveredValueOut)
			*pOptionalHoveredValueOut = ImVec2(0, 0); // Not sure how to initialize this...
		if (window->SkipItems || !values_getter)
			return -1;

		if (precisionInPixels <= 1.f)
			precisionInPixels = 1.f;
		...
		else if (xRange.x == xRange.y)
			xRange.x -= 1.f;
		if (xRange.y == FLT_MAX || xRange.x == xRange.y)
			xRange.y = xRange.x + (rangeExtension.y / inner_bb_extension.y) * inner_bb_extension.x;
		...
		int v_hovered = -1;
		int h_hovered = -1;
		ImVec2 hoveredValue(FLT_MAX, FLT_MAX);
		{
			const bool mustOverrideColors = (pColorsOverride && numColorsOverride > 0);
			const ImU32* col_base         = mustOverrideColors ? pColorsOverride : col_base_embedded;
			const int num_colors          = mustOverrideColors ? numColorsOverride : num_colors_embedded;
			...
			if (!mustOverrideColors)
				col_base_embedded[0] = GetColorU32(ImGuiCol_PlotLines);
			...
			if (numGridLinesHint >= 1.f)
			{
				...
				if (lineSpacing > 0)
				{
					...
					float rangeCoord = floor(yRange.x);
					if (rangeCoord < yRange.x)
					...
					// Draw vertical lines:
					rangeCoord = floor(xRange.x);
					if (rangeCoord < xRange.x)
						rangeCoord += lineSpacing;
			...
			float mouseHoverDeltaX        = 0.f;
			float minDistanceValue        = FLT_MAX;
			float hoveredValueXInBBCoords = -1.f;
			ImU32 curveCol                = 0;
			const float curveThick        = 2.f;
			ImVec2 pos0(0.f, 0.f), pos1(0.f, 0.f);
			...
			{
				if (pOptionalHoveredValueOut)
					*pOptionalHoveredValueOut = hoveredValue;

				// Tooltip on hover
				if (num_curves == 1)
					SetTooltip("P (%1.4f , %1.4f)", hoveredValue.x, hoveredValue.y);
				else
					SetTooltip("P%d (%1.4f , %1.4f)", h_hovered, hoveredValue.x, hoveredValue.y);
	...
	{
		if (item_is_already_present_out)
			*item_is_already_present_out = false;
		if (!txt || txt[0] == '\0' || !items_getter || items_count < 0)
			return -1;
		...
		for (int i = 0; i < items_count; i++)
		{
			if (items_getter(user_data, i, &itxt))
			{
				...
				{
					if (item_is_already_present_out && cmp == 0)
						*item_is_already_present_out = true;
					return i;
				}
			}
		}
	...
	{
		if (!txt || txt[0] == '\0' || !items_getter || !items_inserter || items_count < 0)
			return -1;
		bool alreadyPresent = false;
		if (!item_is_already_present_out)
			item_is_already_present_out = &alreadyPresent;
		...
		{
			if (items_inserter(user_data, itemPosition, txt))
				return itemPosition;
			else
				return -1;
		}
	...
	bool InputTextWithAutoCompletion(const char* label, char* buf, size_t buf_size, InputTextWithAutoCompletionData* pAutocompletion_data, bool (*autocompletion_items_getter)(void*, int, const char**), int autocompletion_items_size, void* autocompletion_user_data, int num_visible_autocompletion_items)
	{
		...
		ad.inited                           = true;
		const ImGuiInputTextFlags itFlags   = (!(ad.newTextToSet.size() > 0 && ad.newTextToSet[0] != '\0')) ? (ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_AutoSelectAll) : (ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_CallbackAlways);
		...
		if (rv)
		{
			...
			if (strlen(buf) > 0)
			{
				...
				{
					ad.itemIndexOfReturnedText = alreadyPresentIndex;
				}
				else
				{
					ad.itemPositionOfReturnedText = itemPlacement;
				}
			}
			return rv;
		}
			...
			if (buf[0] != '\0' && numItems > 0)
			{
				...
				if (ad.lastSelectedTTItemIndex >= 0 && ad.lastSelectedTTItemIndex < numItems && autocompletion_items_getter(autocompletion_user_data, ad.lastSelectedTTItemIndex, &txt))
				{
					...
					if (strcmp(txt, buf) < 0)
					{
						...
						if (i >= numItems)
							selectedTTItemIndex = numItems - 1;
					}
					else
					{
						while (i >= 0)
				...
				if (selectedTTItemIndex + ad.deltaTTItems >= numItems)
					ad.deltaTTItems = numItems - selectedTTItemIndex - 1;
				else if (selectedTTItemIndex + ad.deltaTTItems < 0)
					ad.deltaTTItems = -selectedTTItemIndex;
				...
				{
					useUpperScreen = true;
					if (numTTItems > MaxNumItemsAboveInputText)
						numTTItems = MaxNumItemsAboveInputText;
				}
				else if (numTTItems > MaxNumItemsBelowInputText)
					numTTItems = MaxNumItemsBelowInputText;

				const int numTTItemsHalf = numTTItems / 2;
				int firstTTItemIndex     = selectedTTItemIndex - numTTItemsHalf;
				if (selectedTTItemIndex + numTTItemsHalf >= numItems)
					firstTTItemIndex = numItems - numTTItems;
				if (firstTTItemIndex < 0)
					firstTTItemIndex = 0;
				...
				if (InputTextWithAutoCompletionData::Opacity >= 0.f)
					ImGui::SetNextWindowBgAlpha(InputTextWithAutoCompletionData::Opacity);
	...
	bool InputComboWithAutoCompletion(const char* label, int* current_item, size_t autocompletion_buffer_size, InputComboWithAutoCompletionData* pAutocompletion_data, bool (*items_getter)(void*, int, const char**), // gets item at position ... (cannot be NULL)
					  ...
					  int num_visible_items)
	{
		...
		pad->inited                           = true;
		if (current_item)
			pad->currentAutocompletionItemIndex = *current_item;
		...
		if (!pad->inputTextShown)
		{
			...
			if (comboWidth > buttonsWidth)
				comboWidth -= buttonsWidth;
			else
				noButtons = true;
			...
			if (!noButtons)
			{
				...
				if (InputComboWithAutoCompletionData::ButtonTooltips[0][0] != '\0' && ImGui::IsItemHovered())
					ImGui::SetTooltip("%s", InputComboWithAutoCompletionData::ButtonTooltips[0]);
				...
				if (hasRenameButton)
				{
					...
					if (InputComboWithAutoCompletionData::ButtonTooltips[1][0] != '\0' && ImGui::IsItemHovered())
						ImGui::SetTooltip("%s", InputComboWithAutoCompletionData::ButtonTooltips[1]);
				}
				if (hasDeleteButton)
				{
					ImGui::SameLine(0, 0);
					if (ImGui::Button(InputComboWithAutoCompletionData::ButtonCharcters[2]) && items_deleter(user_data, pad->currentAutocompletionItemIndex))
					{
						...
						if (ImGui::GetIO().KeyShift)
						{
							...
							if (pad->currentAutocompletionItemIndex < 0)
								pad->currentAutocompletionItemIndex = num_items - 1;
						}
						else
							pad->currentAutocompletionItemIndex = -1;
						if (current_item)
							*current_item = pad->currentAutocompletionItemIndex;
					}
					if (InputComboWithAutoCompletionData::ButtonTooltips[2][0] != '\0' && ImGui::IsItemHovered())
						ImGui::SetTooltip("%s", InputComboWithAutoCompletionData::ButtonTooltips[2]);
				}
			}
			// Label
			ImGui::SameLine();
			...
			const ImVec2 label_size = CalcTextSize(label, NULL, true);
			if (label_size.x > 0)
				ImGui::RenderText(ImVec2(window->DC.CursorPos.x, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset), label);
			...
			if (ImGui::ItemAdd(label_bb, labelID))
				pad->itemHovered |= ItemHoverable(label_bb, labelID);
		}
		else
		{
			// ImGui::InputText(...) here
			if (pad->inputTextShown == 1)
			{
				++pad->inputTextShown;
				...
				if (pad->buf[0] != '\0')
				{
					if (pad->isRenaming)
					{
						if (pad->getItemPositionOfReturnedText() >= 0)
						{
							...
							if (oldItemPosition < newItemPositionInOldList)
								newItemPositionInNewList = newItemPositionInOldList - 1;
					...
					if (ImGui::GetIO().KeyShift && !pad->isRenaming)
						mustAllowFurtherEditing = true;
					pad->buf[0]         = '\0';
					pad->inputTextShown = mustAllowFurtherEditing ? 1 : 0;
					pad->isRenaming     = false;
				}
...
namespace ImGui
{
	struct MyTreeViewHelperStruct
	{
		...
		struct ContextMenuDataStruct
		{
			...
			ContextMenuDataStruct()
			    : activeNode(NULL), parentTreeView(NULL), activeNodeChanged(false)
			{
			}
			void reset()
			{
				*this = ContextMenuDataStruct();
			}
		};
		...
		struct NameEditingStruct
		{
			...
			NameEditingStruct(TreeViewNode* n = NULL, const char* startingText = NULL)
			    : editingNode(n)
			{
				textInput[0] = '\0';
				if (startingText)
				{
					...
					if (len < 255)
						strcpy(&textInput[0], startingText);
					else
					{
			...
			void reset()
			{
				*this = NameEditingStruct();
			}
		};
		static NameEditingStruct NameEditing;
		MyTreeViewHelperStruct(TreeView& parent, TreeViewNode::Event& _event)
		    : parentTreeView(parent), mustDrawAllNodesAsDisabled(false), event(_event)
		{
		...
		void fillEvent(TreeViewNode* _eventNode = NULL, TreeViewNode::State _eventFlag = TreeViewNode::STATE_NONE, bool _eventFlagRemoved = false)
		{
			...
			if (event.state != TreeViewNode::STATE_NONE)
				event.type = TreeViewNode::EVENT_STATE_CHANGED;
		}
		// Sorters
		...
		static void Serialize(ImGuiHelper::Serializer& s, TreeViewNode* n)
		{
			if (n->parentNode)
			{
				...
				if (n->data.tooltip)
					s.saveTextLines(n->data.tooltip, "tooltip");
				if (n->data.userText)
					s.saveTextLines(n->data.userText, "userText");
				if (n->data.userId)
					s.save(&n->data.userId, "userId");
			...
			else
			{
				...
				int tmp = (int)tv.selectionMode;
				s.save(&tmp, "selectionMode");
				s.save(&tv.allowMultipleSelection, "allowMultipleSelection");
				tmp = (int)tv.checkboxMode;
				s.save(&tmp, "checkboxMode");
		...
		static bool ParseCallback(ImGuiHelper::FieldType /*ft*/, int /*numArrayElements*/, void* pValue, const char* name, void* userPtr)
		{
			...
			if (strcmp(name, "name") == 0)
				ImGuiHelper::StringAppend(n->data.displayName, (const char*)pValue, false);
			else if (strcmp(name, "tooltip") == 0)
				ImGuiHelper::StringAppend(n->data.tooltip, (const char*)pValue, true);
			else if (strcmp(name, "userText") == 0)
				ImGuiHelper::StringAppend(n->data.userText, (const char*)pValue, true);
			else if (strcmp(name, "userId") == 0)
				n->getUserId() = *((int*)pValue);
			else if (strcmp(name, "state") == 0)
				n->state = *((int*)pValue);
			else if (strcmp(name, "numChildNodes") == 0)
			{
				cbs.numChildNodes = *((int*)pValue);
				return true;
			}
			return false;
		}
		static bool ParseTreeViewDataCallback(ImGuiHelper::FieldType /*ft*/, int /*numArrayElements*/, void* pValue, const char* name, void* userPtr)
		{
			...
			if (strcmp(name, "stateColors0") == 0)
				tv.stateColors[0] = *((ImVec4*)pValue);
			else if (strcmp(name, "stateColors1") == 0)
				tv.stateColors[1] = *((ImVec4*)pValue);
			else if (strcmp(name, "stateColors2") == 0)
				tv.stateColors[2] = *((ImVec4*)pValue);
			else if (strcmp(name, "stateColors3") == 0)
				tv.stateColors[3] = *((ImVec4*)pValue);
			else if (strcmp(name, "stateColors4") == 0)
				tv.stateColors[4] = *((ImVec4*)pValue);
			else if (strcmp(name, "stateColors5") == 0)
				tv.stateColors[5] = *((ImVec4*)pValue);
			else if (strcmp(name, "selectionMode") == 0)
				tv.selectionMode = *((int*)pValue);
			else if (strcmp(name, "allowMultipleSelection") == 0)
				tv.allowMultipleSelection = *((bool*)pValue);
			else if (strcmp(name, "checkboxMode") == 0)
				tv.checkboxMode = *((int*)pValue);
			else if (strcmp(name, "allowAutoCheckboxBehaviour") == 0)
				tv.allowAutoCheckboxBehaviour = *((bool*)pValue);
			else if (strcmp(name, "collapseToLeafNodesAtNodeDepth") == 0)
				tv.collapseToLeafNodesAtNodeDepth = *((int*)pValue);
			else if (strcmp(name, "inheritDisabledLook") == 0)
			{
				tv.inheritDisabledLook = *((bool*)pValue);
				return true;
			}
			return false;
		}
			...
			if (cbs.numChildNodes >= 0)
			{
				...
				else
				{
					for (int i = 0; i < cbs.numChildNodes; i++)
					{
						...
						if (callback)
							callback(node, *tv, false, tv->treeViewNodeCreationDelationCbUserPtr);
					}
				}
			}
		}
		...
		static void TreeViewNodePopupMenuProvider(TreeViewNode* n, TreeView& tv, void*)
		{
			...
			if ((canBeSetToDefault || canBeEnabledOrDisabled || itsChildNodesAreSortable || canBeRenamed || canMoveUp || canMoveDown || canAddSiblings || canAddChildNodes || canDeleteNode || canForceCheckboxVisibility || canBeHidden) && ImGui::BeginPopup(TreeView::GetTreeViewNodePopupMenuName()))
			{
				ImGui::PushID(n);
				...
				if (canBeEnabledOrDisabled)
				{
					...
					{
						if (ImGui::MenuItem("Enable item"))
							n->removeState(TreeViewNode::STATE_DISABLED);
					}
					else if (ImGui::MenuItem("Disable item"))
						n->addState(TreeViewNode::STATE_DISABLED);
				}
				if (itsChildNodesAreSortable)
				{
					if (ImGui::MenuItem("Sort child nodes in acending order"))
						n->sortChildNodesByDisplayName(false, false);
					if (ImGui::MenuItem("Sort child nodes in descending order"))
						n->sortChildNodesByDisplayName(false, true);
				}
				if (canBeRenamed && ImGui::MenuItem("Rename"))
				{
					n->startRenamingMode();
				}
				if (canMoveUp && ImGui::MenuItem("Move up"))
					n->moveNodeTo(nodeIndex - 1);
				if (canMoveDown && ImGui::MenuItem("Move down"))
					n->moveNodeTo(nodeIndex + 1);
				...
				if (canDeleteNode && ImGui::MenuItem("Delete"))
				{
					TreeViewNode::DeleteNode(n);
					n = NULL;
				}
				if (canForceCheckboxVisibility && ImGui::MenuItem("Toggle force checkbox"))
					n->toggleState(TreeViewNode::STATE_FORCE_CHECKBOX);
				if (canBeHidden && ImGui::MenuItem("Hide"))
					n->addState(TreeViewNode::STATE_HIDDEN);
				ImGui::PopID();
				ImGui::EndPopup();
			}
		}
	...
	{
		if (this == MyTreeViewHelperStruct::ContextMenuData.activeNode)
			MyTreeViewHelperStruct::ContextMenuData.reset();
		...
		if (parentNode)
		{
			...
			if (tv.treeViewNodeCreationDelationCb)
				tv.treeViewNodeCreationDelationCb(this, tv, true, tv.treeViewNodeCreationDelationCbUserPtr);
		}
		parentNode = NULL;
	}

	TreeViewNode::TreeViewNode(const Data& _data, TreeViewNode* _parentNode, int nodeIndex, bool addEmptyChildNodeVector)
	    : userPtr(NULL), data(_data)
	{
		parentNode = NULL;
		childNodes = NULL;
		state      = 0;
		...
		if (parentNode)
		{
			...
			if (!parentNode->childNodes)
			{
				...
				IM_PLACEMENT_NEW(parentNode->childNodes)
				ImVector<TreeViewNode*>();
		...
		if (parentNode)
		{
			TreeView& tv = getTreeView();
			if (tv.treeViewNodeCreationDelationCb)
				tv.treeViewNodeCreationDelationCb(this, tv, false, tv.treeViewNodeCreationDelationCbUserPtr);
		}
	}
	...
	TreeViewNode* TreeViewNode::CreateNode(const Data& _data, TreeViewNode* _parentNode, int nodeIndex, bool addEmptyChildNodeVector)
	{
		...
		IM_PLACEMENT_NEW(n)
		TreeViewNode(_data, _parentNode, nodeIndex, addEmptyChildNodeVector);
		return n;
	}
	...
	TreeViewNode* TreeViewNode::getParentNode()
	{
		return (parentNode && parentNode->parentNode) ? parentNode : NULL;
	}

	const TreeViewNode* TreeViewNode::getParentNode() const
	{
		return (parentNode && parentNode->parentNode) ? parentNode : NULL;
	}
	...
	void TreeViewNode::moveNodeTo(int nodeIndex)
	{
		...
		if (curNodeIndex < nodeIndex)
		{
			for (int i = curNodeIndex; i < nodeIndex; i++)
	...
	void TreeViewNode::deleteAllChildNodes(bool leaveEmptyChildNodeVector)
	{
		if (childNodes && childNodes->size() > 0)
		{
			while (childNodes->size() > 0)
				DeleteNode((*childNodes)[0]);
			childNodes->clear();
		}
		if (!childNodes)
		{
			if (leaveEmptyChildNodeVector)
				addEmptyChildNodeVector();
		}
		else if (childNodes->size() == 0 && !leaveEmptyChildNodeVector)
			removeEmptyChildNodeVector();
	}
	...
	{
		if (!parentNode || !parentNode->childNodes || nodeIndexInParentHierarchy < 0 || nodeIndexInParentHierarchy >= parentNode->childNodes->size())
			return NULL;
	...
	void TreeViewNode::Swap(TreeViewNode*& n1, TreeViewNode*& n2)
	{
		...
		if (!n1 || !n2)
			return;
		{
			TreeViewNode* tmp = n1->parentNode;
			n1->parentNode    = n2->parentNode;
			n2->parentNode    = tmp;
		}
		{
			ImVector<TreeViewNode*>* tmp = n1->childNodes;
		...
		if (childNodes && childNodes->size() > 1)
		{
			if (recursive)
			{
				for (int i = 0, isz = childNodes->size(); i < isz; i++)
					(*childNodes)[i]->sortChildNodes(recursive, comp);
			}
	...
	void TreeViewNode::addStateToAllChildNodes(int stateFlag, bool recursive) const
	{
		if (childNodes)
		{
			...
			{
				if (recursive)
					(*childNodes)[i]->addStateToAllChildNodes(stateFlag, recursive);
	...
	void TreeViewNode::removeStateFromAllChildNodes(int stateFlag, bool recursive) const
	{
		if (childNodes)
		{
			...
			{
				if (recursive)
					(*childNodes)[i]->removeStateFromAllChildNodes(stateFlag, recursive);
				(*childNodes)[i]->state &= (~stateFlag);
			}
		}
	}
	...
	bool TreeViewNode::isStatePresentInAllChildNodes(int stateFlag) const
	{
		if (childNodes)
		{
			for (int i = 0, isz = childNodes->size(); i < isz; i++)
			{
				if (((*childNodes)[i]->state & stateFlag) != stateFlag)
					return false;
			}
		}
		return true;
	...
	bool TreeViewNode::isStatePresentInAllDescendants(int stateFlag) const
	{
		if (childNodes)
		{
			for (int i = 0, isz = childNodes->size(); i < isz; i++)
			{
				if (((*childNodes)[i]->state & stateFlag) != stateFlag || !(*childNodes)[i]->isStatePresentInAllDescendants(stateFlag))
					return false;
			}
		}
		return true;
	...
	bool TreeViewNode::isStateMissingInAllDescendants(int stateFlag) const
	{
		if (childNodes)
		{
			for (int i = 0, isz = childNodes->size(); i < isz; i++)
			{
				if (((*childNodes)[i]->state & stateFlag) == stateFlag || !(*childNodes)[i]->isStateMissingInAllDescendants(stateFlag))
					return false;
			}
		}
		return true;
	...
	TreeViewNode* TreeViewNode::getFirstParentNodeWithState(int stateFlag, bool recursive)
	{
		...
		while (n && n->parentNode)
		{
			if ((n->state & stateFlag) == stateFlag)
				return n;
			if (!recursive)
				break;
			n = n->parentNode;
	...
	void TreeViewNode::getAllChildNodesWithState(ImVector<TreeViewNode*>& result, int stateFlag, bool recursive, bool returnOnlyLeafNodes, bool clearResultBeforeUsage) const
	{
		...
		if (childNodes)
		{
			for (int i = 0, isz = childNodes->size(); i < isz; i++)
			{
				...
				if ((n->state & stateFlag) == stateFlag && (!returnOnlyLeafNodes || n->isLeafNode()))
					result.push_back(const_cast<TreeViewNode*>(n));
				if (recursive)
					n->getAllChildNodesWithState(result, stateFlag, recursive, returnOnlyLeafNodes, false);
			}
		}
	}
	...
	void TreeViewNode::getAllChildNodes(ImVector<TreeViewNode*>& result, bool recursive, bool returnOnlyLeafNodes, bool clearResultBeforeUsage) const
	{
		...
		if (childNodes)
		{
			...
			{
				const TreeViewNode* n = (*childNodes)[i];
				if (!returnOnlyLeafNodes || n->isLeafNode())
					result.push_back(const_cast<TreeViewNode*>(n));
				if (recursive)
					n->getAllChildNodes(result, recursive, returnOnlyLeafNodes, false);
			}
		}
	}
	...
	TreeView::TreeView(Mode _selectionMode, bool _allowMultipleSelection, Mode _checkboxMode, bool _allowAutoCheckboxBehaviour, bool _inheritDisabledLook)
	    : TreeViewNode(Data(), NULL, -1, true)
	{
		...
		stateColors[0] = ImVec4(1.f, 0.f, 0.f, 1.f);
		stateColors[1] = stateColors[0];
		stateColors[1].w *= 0.4f;
		stateColors[2] = ImVec4(0.f, 1.f, 0.f, 1.f);
		stateColors[3] = stateColors[2];
		stateColors[3].w *= 0.4f;
		stateColors[4] = ImVec4(0.f, 0.f, 1.f, 1.f);
		stateColors[5] = stateColors[4];
		stateColors[5].w *= 0.4f;
	}

	TreeView::~TreeView()
	{
		if (this == MyTreeViewHelperStruct::ContextMenuData.parentTreeView)
			MyTreeViewHelperStruct::ContextMenuData.reset();
	}
	...
	void TreeViewNode::render(void* ptr, int numIndents)
	{
		...
		if (!mustSkipToLeafNodes)
		{
			...
			ImGui::PushID(this);
			if (allowCheckBox && !tvhs.hasCbGlyphs)
				ImGui::AlignTextToFramePadding();
			...
			if (allowCheckBox)
			{
				...
				if (!tvhs.hasCbGlyphs)
					checkedChanged = ImGui::Checkbox("###chb", &statechecked);
				else
				{
				...
				if (checkedChanged)
				{
					...
					if (tv.allowAutoCheckboxBehaviour)
					{
						...
						{
							if (statechecked)
								addStateToAllDescendants(STATE_CHECKED);
							else if (isStatePresentInAllDescendants(STATE_CHECKED))
								removeStateFromAllDescendants(STATE_CHECKED);
						}
						TreeViewNode* p = parentNode;
						while (p && p->parentNode != NULL)
						{
							if (!statechecked)
								p->removeState(STATE_CHECKED);
							else if (!(p->state & STATE_CHECKED))
							{
								if (p->isStatePresentInAllDescendants(STATE_CHECKED))
									p->addState(STATE_CHECKED);
							}
							p = p->parentNode;
						}
					}
			...
			if (tv.treeViewNodeDrawIconCb)
				itemHovered |= tv.treeViewNodeDrawIconCb(this, tv, tv.treeViewNodeDrawIconCbUserPtr);
			...
			if (this != MyTreeViewHelperStruct::NameEditing.editingNode)
			{
				if (mustdrawdisabled)
				{
					if (!customColorState)
						...
						ImGui::PushStyleColor(ImGuiCol_Text, tv.stateColors[(customColorState - 1) * 2 + 1]);
				}
				if ((state & STATE_DEFAULT) || (allowSelection && stateselected))
				{
					...
					{
						const ImVec2 textSize = ImGui::CalcTextSize(data.displayName);
				...
				if (!customColorState || mustdrawdisabled)
					ImGui::Text("%s", data.displayName);
				else
					ImGui::TextColored(tv.stateColors[(customColorState - 1) * 2], "%s", data.displayName);
				...
				{
					itemHovered = true;
					if (data.tooltip && strlen(data.tooltip) > 0)
						ImGui::SetTooltip("%s", data.tooltip);
				}
			}
			else
			{
				...
				{
					if (!mustFocus && !ImGui::IsItemActive())
						MyTreeViewHelperStruct::NameEditing.reset();
					mustFocus = false;
				}
			}
			if (tv.treeViewNodeAfterDrawCb)
				tv.treeViewNodeAfterDrawCb(this, tv, tvhs.windowWidth, tv.treeViewNodeAfterDrawCbUserPtr);
			...
			{
				if (!(ImGui::GetIO().KeyCtrl && tv.allowMultipleSelection))
					tv.removeStateFromAllDescendants(STATE_SELECTED);
		...
		if (isStatePresent(STATE_OPEN) || mustSkipToLeafNodes)
		{
			...
			for (int i = 0, isz = childNodes->size(); i < isz; i++)
			{
				...
				if (n)
				{
					...
					if (tv.inheritDisabledLook && (state & STATE_DISABLED))
						tvhs.mustDrawAllNodesAsDisabled = true;
			...
			if (cmd.activeNode && cmd.parentTreeView && cmd.parentTreeView->treeViewNodePopupMenuDrawerCb)
			{
				if (cmd.activeNodeChanged)
				{
					...
					while (g.OpenPopupStack.size() > 0)
						g.OpenPopupStack.pop_back(); // Close all existing context-menus
	...
	void TreeView::clear()
	{
		TreeViewNode::deleteAllChildNodes(true);
	}

	ImVec4* TreeView::getTextColorForStateColor(int aStateColorFlag) const
	{
		if (aStateColorFlag & STATE_COLOR1)
			return &stateColors[0];
		if (aStateColorFlag & STATE_COLOR2)
			return &stateColors[2];
		if (aStateColorFlag & STATE_COLOR3)
	...
	void TreeView::setTextColorForStateColor(int aStateColorFlag, const ImVec4& textColor, float disabledTextColorAlphaFactor) const
	{
		if (aStateColorFlag & STATE_COLOR1)
		{
			stateColors[0]   = textColor;
			stateColors[1]   = stateColors[0];
			stateColors[1].w = stateColors[0].w * disabledTextColorAlphaFactor;
		}
		if (aStateColorFlag & STATE_COLOR2)
		{
			stateColors[2]   = textColor;
			stateColors[3]   = stateColors[2];
			stateColors[3].w = stateColors[2].w * disabledTextColorAlphaFactor;
		}
		if (aStateColorFlag & STATE_COLOR3)
		{
			stateColors[4]   = textColor;
			stateColors[5]   = stateColors[4];
			stateColors[5].w = stateColors[4].w * disabledTextColorAlphaFactor;
		}
	}
...
//-------------------------------------------------------------------------------
	...
	bool TreeView::load(ImGuiHelper::Deserializer& d, const char** pOptionalBufferStart)
	{
		...
		if (pOptionalBufferStart)
			*pOptionalBufferStart = amount;
	...
	void TreeView::SetFontCheckBoxGlyphs(const char* emptyState, const char* fillState)
	{
		if (emptyState && strlen(emptyState) > 0 && strlen(emptyState) < 5 && fillState && strlen(fillState) > 0 && strlen(fillState) < 5)
		{
			...
			FontCheckBoxGlyphs[0][0] = FontCheckBoxGlyphs[1][0] = '\0';
	}
	void TreeView::SetFontArrowGlyphs(const char* leftArrow, const char* downArrow)
	{
		if (leftArrow && strlen(leftArrow) > 0 && strlen(leftArrow) < 5 && downArrow && strlen(downArrow) > 0 && strlen(downArrow) < 5)
		{
			...
			FontArrowGlyphs[0][0] = FontArrowGlyphs[1][0] = '\0';
	}
...
namespace ImGui
{
	...
	inline static ImU32 PasswordDrawerLighten(ImU32 color, int amount)
	{
		...
		for (int i = 0; i < 3; i++)
			rgba[0] = ImClamp(rgba[i] + amount, 0, 255);
	...
	bool PasswordDrawer(char* password, int passwordSize, ImGuiPasswordDrawerFlags flags, const float size, const ImU32 colors[7])
	{
		...
		ImGuiWindow* window = GetCurrentWindow();
		if (window->SkipItems || passwordSize < 5)
			return false;
		...
		else
		{
			ImU32 bgColors[4];
			for (int i = 0; i < 4; i++)
				bgColors[i] = PasswordDrawerLighten(pColors[i], (pressed || held) ? 60 : 30);
		...
		for (int i = 4; i >= 0; --i)
		{
			...
			{
				numRows = 2 + i;
				if (passwordLen > numRowsSquared[i])
					passwordLen = numRowsSquared[i]; // Max 36
				break;
			}
		}
	...
	unsigned int CheckboxFlags(const char* label, unsigned int* flags, int numFlags, int numRows, int numColumns, unsigned int flagAnnotations, int* itemHoveredOut, const unsigned int* pFlagsValues)
	{
		...
		if (itemHoveredOut)
			*itemHoveredOut = -1;
		if (numRows * numColumns * numFlags == 0)
			return itemPressed;
		...
		int numItemsPerRow = numFlags / numRows;
		if (numFlags % numRows)
			++numItemsPerRow;
		int numItemsPerGroup = numItemsPerRow / numColumns;
		if (numItemsPerRow % numColumns)
			++numItemsPerGroup;
		...
		if (flagAnnotations)
		{
			...
			annSegments = (int)(checkSize.x * 0.4f);
			if (annSegments < 3)
				annSegments = 3;
		...
		for (int i = 0; i < numFlags; i++)
		{
			...
			if (v)
			{
				...
				if (vNotif)
					window->DrawList->AddCircle(bb.Min + ImVec2(annCenter, annCenter), annRadius, annColor, annSegments, annThickness);
			}
			else if (vNotif)
				window->DrawList->AddCircle(bb.Min + ImVec2(annCenter, annCenter), annRadius, annColor, annSegments, annThickness);
	...
	bool CheckboxStyled(const char* label, bool* v, const ImU32* pOptionalEightColors, const ImVec2& checkBoxScale, float checkBoxRounding)
	{
		...
		if (pressed)
		{
			*v = !(*v); // change state soon
			if (timeID == id)
			{
				...
				if (elapsedTime > timeActionLength)
					timeBegin = ImGui::GetTime(); // restart
				else
				{
		...
		rounding *= innerFrameHeight * 0.065f;
		if (rounding > 16.f)
			rounding = 16.f;
		...
		if (t > 0)
		{
			ImU32 fillColor0 = pOptionalEightColors ? ((held || hovered) ? pOptionalEightColors[5] : pOptionalEightColors[4]) : (GetColorU32((held || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button));
		...
		if (t < 1)
		{
			ImU32 fillColor1 = pOptionalEightColors ? ((held || hovered) ? pOptionalEightColors[7] : pOptionalEightColors[6]) : (GetColorU32((held || hovered) ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg));
		...
		int numSegments = (int)(check_bb_height * 0.8f);
		if (numSegments < 3)
			numSegments = 3;
		else if (numSegments > 24)
			numSegments = 24;
		float radius = check_bb_height * 0.5f;
		if (widthFraction < radius)
			widthFraction = radius;
		else if (widthFraction > check_bb_width - radius)
			widthFraction = check_bb_width - radius;
		...
		if (!animationActive)
		{
			if (*v)
			{
				ImU32 circleColorOn = pOptionalEightColors ? ((held || hovered) ? pOptionalEightColors[1] : pOptionalEightColors[0]) : (GetColorU32((held || hovered) ? defaultCircleColorOnHovered : defaultCircleColorOn));
				int col             = (circleColorOn & ~IM_COL32_A_MASK) | (0xFF << IM_COL32_A_SHIFT);
				window->DrawList->AddCircleFilled(center, radius, col, numSegments);
			}
			else
			{
				ImU32 circleColorOff = pOptionalEightColors ? ((held || hovered) ? pOptionalEightColors[3] : pOptionalEightColors[2]) : (GetColorU32((held || hovered) ? defaultCircleColorOffHovered : defaultCircleColorOff));
		...
		{
			int col1 = (int)(pOptionalEightColors ? ((held || hovered) ? pOptionalEightColors[1] : pOptionalEightColors[0]) : (GetColorU32((held || hovered) ? defaultCircleColorOnHovered : defaultCircleColorOn)));
			int col0 = (int)(pOptionalEightColors ? ((held || hovered) ? pOptionalEightColors[3] : pOptionalEightColors[2]) : (GetColorU32((held || hovered) ? defaultCircleColorOffHovered : defaultCircleColorOff)));
	...
	void LoadingIndicatorCircle(const char* label, float indicatorRadiusFactor, const ImVec4* pOptionalMainColor, const ImVec4* pOptionalBackdropColor, int circle_count, const float speed)
	{
		ImGuiWindow* window = GetCurrentWindow();
		if (window->SkipItems)
		{
		...
		if (circle_count <= 0)
			circle_count = 12;
		if (indicatorRadiusFactor <= 0.f)
			indicatorRadiusFactor = 1.f;
		if (!pOptionalMainColor)
			pOptionalMainColor = &style.Colors[ImGuiCol_Button];
		if (!pOptionalBackdropColor)
			pOptionalBackdropColor = &style.Colors[ImGuiCol_ButtonHovered];
		...
		const ImRect bb(pos, ImVec2(pos.x + indicatorRadiusPixels * 2.f + 4.f * circle_radius, pos.y + indicatorRadiusPixels * 2.f + 4.f * circle_radius));
		ItemSize(bb, style.FramePadding.y);
		if (!ItemAdd(bb, id))
		{
			return;
	...
	void LoadingIndicatorCircle2(const char* label, float indicatorRadiusFactor, float indicatorRadiusThicknessFactor, const ImVec4* pOptionalColor)
	{
		...
		if (indicatorRadiusFactor <= 0.f)
			indicatorRadiusFactor = 1.f;
		if (indicatorRadiusThicknessFactor <= 0.f)
			indicatorRadiusThicknessFactor = 1.f;
		if (!pOptionalColor)
			pOptionalColor = &style.Colors[ImGuiCol_Button];
		...
		if (indicatorThicknessPixels > indicatorThicknessPixels * 0.4f)
			indicatorThicknessPixels = indicatorThicknessPixels * 0.4f;
