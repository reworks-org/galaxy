...
#ifndef GALAXY_AUDIO_PLAYLIST_HPP_
#define GALAXY_AUDIO_PLAYLIST_HPP_
...
namespace galaxy
{
	namespace audio
	{
		...
		public:
			...
			///
			Playlist(const nlohmann::json& json) noexcept;
		...
		template<is_audio Audio>
		inline Playlist<Audio>::Playlist(const nlohmann::json& json) noexcept
		{
		...
		template<is_audio Audio>
		inline void Playlist<Audio>::add(std::string_view file) noexcept
		{
			std::lock_guard<std::mutex> lock {m_mutex};
		...
		template<is_audio Audio>
		inline void Playlist<Audio>::shuffle() noexcept
		{
