...
namespace qs
{
	    ...
	    : VertexData {}, m_update_renderdata {false}, m_offset {0}, m_max_quads {max_quads}, m_max_vertexs {0}, m_max_indexs {0}, m_used_indexs {0}, m_texture {nullptr}
	{
		...
		m_vb.create<qs::BatchedVertex, qs::BufferDynamic>(m_vertexs);
		m_ib.create<qs::BufferStatic>(is);

		m_layout.add<qs::BatchedVertex, qs::VAPosition>(2);
		m_layout.add<qs::BatchedVertex, qs::VATexel>(2);
		m_layout.add<qs::BatchedVertex, qs::VAOpacity>(1);
	...
	void SpriteBatch::update()
	{
		...
		{
			glNamedBufferSubData(m_vb.id(), 0, sizeof(qs::BatchedVertex) * m_vertexs.size(), m_vertexs.data());
