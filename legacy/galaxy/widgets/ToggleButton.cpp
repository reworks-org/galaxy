...
namespace galaxy
{
	namespace ui
	{
		...
		{
			m_sprite.create(on, ORTHO_FAR_24BIT);
			m_regions[0] = on;
			m_regions[1] = off;
			m_regions[2] = on_hover;
			m_regions[3] = off_hover;

			m_bounds.m_width  = m_sprite.get_region().m_width;
			m_bounds.m_height = m_sprite.get_region().m_height;

			m_theme->m_sb.add(&m_sprite, &m_transform);
		...
		void ToggleButton::update(const double dt)
		{
			switch (m_state)
			{
				case ToggleButton::State::ON:
					m_sprite.update_region(m_regions[0]);
					break;

				case ToggleButton::State::OFF:
					m_sprite.update_region(m_regions[1]);
					break;

				case ToggleButton::State::ON_HOVER:
					m_sprite.update_region(m_regions[2]);
					break;

				case ToggleButton::State::OFF_HOVER:
					m_sprite.update_region(m_regions[3]);
					break;
			}
		}
