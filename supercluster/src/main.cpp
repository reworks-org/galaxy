...
{
public:
	Supercluster(std::unique_ptr<galaxy::fs::Config>& config)
	    : Application(config)
	{
	}
};
	...
	do
	{
		...
		try
		{
			...
			if (!config->open())
			{
				...
				config->define<std::string>("window-name", "Supercluster Editor");
