...
namespace ImGui
{
	namespace SFML
	{
		...
		void Update(sf::Window& window, sf::RenderTarget& target)
		{
			...
			if (!s_currWindowCtx->mouseMoved)
			{
				...
				Update(s_currWindowCtx->touchPos, sf::Vector2f(target.getSize()));
			}
			else
			{
				Update(sf::Mouse::getPosition(window), sf::Vector2f(target.getSize()));
			}
		}
		...
		{
			assert(s_currWindowCtx);
			s_currWindowCtx->allowInput = true;
		}

		void DisableInput()
		{
			assert(s_currWindowCtx);
			s_currWindowCtx->allowInput = false;
		}

		bool IsInputAllowed()
		{
			assert(s_currWindowCtx);
			return s_currWindowCtx->allowInput;
		}
	...
	bool ImageButton(const char* id, const sf::RenderTexture& texture, const sf::Vector2f& size, const sf::Color& bgColor, const sf::Color& tintColor)
	{
		ImTextureID textureID = convertGLTextureHandleToImTextureID(texture.getTexture().getNativeHandle());
