...
namespace galaxy
{
	namespace systems
	{
		...
		{
			scene->m_world.operate<components::Transform2D, components::Renderable>(std::execution::par, [&](const ecs::Entity entity, components::Transform2D* transform, components::Renderable* renderable) {
				if (transform->is_dirty())
				{
					...
					switch (renderable->m_type)
					{
						case graphics::Renderables::BATCHED:
						{
							...
							renderable->update_aabb({transform->get_pos().x, transform->get_pos().y, batchsprite->get_region().m_width, batchsprite->get_region().m_height});
						}
						break;
						...
						case graphics::Renderables::SPRITE:
						{
							...
							renderable->update_aabb({transform->get_pos().x, transform->get_pos().y, static_cast<float>(sprite->get_width()), static_cast<float>(sprite->get_height())});
						}
						break;
						...
						case graphics::Renderables::TEXT:
						{
							...
							renderable->update_aabb({transform->get_pos().x, transform->get_pos().y, static_cast<float>(text->get_width()), static_cast<float>(text->get_height())});
						}
						break;
						...
						case graphics::Renderables::LINE_LOOP:
						{
							...
							renderable->update_aabb({transform->get_pos().x, transform->get_pos().y, static_cast<float>(primitive->get_width()), static_cast<float>(primitive->get_height())});
