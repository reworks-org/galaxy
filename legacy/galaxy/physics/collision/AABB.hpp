///
/// AABB.hpp
/// galaxy
///
/// Refer to LICENSE.txt for more details.
///

#ifndef GALAXY_PHYSICS_COLLISION_AABB_HPP_
#define GALAXY_PHYSICS_COLLISION_AABB_HPP_

#include <glm/vec2.hpp>

namespace galaxy
{
	namespace physics
	{
		///
		/// AABB bounding box for seperating axis theorm.
		/// See:
		/// https://www.azurefromthetrenches.com/introductory-guide-to-aabb-tree-collision-detection/
		/// https://realtimecollisiondetection.net/
		///
		struct AABB final
		{
			///
			/// Constructor.
			///
			AABB() noexcept;

			///
			/// Argument constructor.
			///
			/// \param min Minimum bounds.
			/// \param max Maximum bounds.
			///
			AABB(const glm::vec2& min, const glm::vec2& max) noexcept;

			///
			/// Destructor.
			///
			~AABB() noexcept = default;

			///
			/// Test for an overlap.
			///
			/// \param b AABB to test.
			///
			/// \return Const bool. True if overlap.
			///
			[[nodiscard]] const bool overlaps(const AABB& b) const noexcept;

			///
			/// Check if this AABB contains another AABB.
			///
			/// \param b AABB to check.
			///
			/// \return Const bool. True if contains.
			///
			[[nodiscard]] const bool contains(const AABB& b) const noexcept;

			///
			/// Merge two AABBs.
			///
			/// \param b AABB to merge with this.
			///
			/// \return Merged AABB.
			///
			[[nodiscard]] AABB merge(const AABB& b) const noexcept;

			///
			/// Get intersection of two AABBs.
			///
			/// \param b AABB to test.
			///
			/// \return AABB intersection.
			///
			[[nodiscard]] AABB intersection(const AABB& b) const noexcept;

			///
			/// Get surface area of AABB.
			///
			/// \return Const float.
			///
			[[nodiscard]] const float surface_area() const noexcept;

			///
			/// Lower bounds.
			///
			glm::vec2 m_min;

			///
			/// Upper bounds.
			///
			glm::vec2 m_max;
		};
	} // namespace physics
} // namespace galaxy

#endif