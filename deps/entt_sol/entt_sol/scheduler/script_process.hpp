...
namespace entt_sol
{
	...
	public:
		...
		script_process(const allocator_type& allocator, const sol::table& t, const fsec freq = std::chrono::milliseconds {250})
			: basic_process {allocator}
			, m_self {t}
			, m_frequency {freq}
		{
...
#undef BIND
		...
		~script_process()
		{
			std::cout << "script_process: " << m_self.pointer() << " terminated" << std::endl;
			m_self.clear();
			m_self.abandon();
		}
		...
		void init()
		{
			std::cout << "script_process: " << m_self.pointer() << " joined" << std::endl;
			_call("init");
		}
	...
	[[nodiscard]]
	inline sol::table open_scheduler(sol::this_state s)
	{
