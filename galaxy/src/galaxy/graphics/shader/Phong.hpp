...
namespace galaxy
{
	namespace shaders
	{
		...
		inline const std::string phong_frag = R"(
            ...
            vec3 calc_directional(DirLight light, vec3 normal, vec3 view_dir, vec3 diff_map, vec3 spec_map);
            vec3 calc_point(PointLight light, vec3 normal, vec3 frag_pos, vec3 view_dir, vec3 diff_map, vec3 spec_map);
            vec3 calc_spot(SpotLight light, vec3 normal, vec3 frag_pos, vec3 view_dir, vec3 diff_map, vec3 spec_map);
            ...
            void main()
            {
                ...
                vec4 diff_tex = texture(tex_DIFFUSE1, io_texels);
                vec3 diff_map = pow(diff_tex.rgb, vec3(GAMMA_CORRECTION));
            ...
            vec3 calc_directional(DirLight light, vec3 normal, vec3 view_dir, vec3 diff_map, vec3 spec_map)
            {
                ...
                //vec3 ambient = light.ambient_intensity * (material.ambient * diff_map);
                //vec3 diffuse = light.diffuse_intensity * diff * (material.diffuse * diff_map);
                //vec3 specular = light.specular_intensity * spec * (material.specular * spec_map);

                vec3 ambient = light.ambient_intensity * diff_map;
                vec3 diffuse = light.diffuse_intensity * diff * diff_map;
