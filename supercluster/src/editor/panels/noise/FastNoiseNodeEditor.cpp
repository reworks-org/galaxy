...
namespace sc
{
	...
	void FastNoiseNodeEditor::DoNodes()
	{
		for (auto& node : mNodes)
		{
			...
			ui::imgui_image(node.second.noiseTexture, noiseSize);
	...
	FastNoise::OutputMinMax FastNoiseNodeEditor::GenerateNodePreviewNoise(FastNoise::Generator* gen, float* noise)
	{
		switch (mNodeGenType)
		{
			...
			case NoiseTexture::GenType_3D:
				return gen
					->GenUniformGrid3D(noise, Node::NoiseSize / -2, Node::NoiseSize / -2, 0, Node::NoiseSize, Node::NoiseSize, 1, mNodeFrequency, mNodeSeed);
