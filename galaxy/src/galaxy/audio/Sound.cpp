...
namespace galaxy
{
	namespace audio
	{
		...
		Sound::Sound(Sound::Type type, std::string_view file) noexcept
			: m_sound {}
		{
			load(type, file);
		}
		...
		void Sound::load(Sound::Type type, std::string_view file) noexcept
		{
		...
		void Sound::toggle_spatialization(const bool enable) noexcept
		{
		...
		void Sound::set_looping(const bool loop) noexcept
		{
		...
		void Sound::fade_in(const ma_uint64 ms) noexcept
		{
			ma_sound_set_fade_in_milliseconds(&m_sound, 0.0f, 1.0f, ms);
		}

		void Sound::fade_out(const ma_uint64 ms) noexcept
		{
		...
		void Sound::set_pinned_listener(const unsigned int id) noexcept
		{
			ma_sound_set_pinned_listener_index(&m_sound, id);
		}

		void Sound::set_positioning(ma_positioning positioning) noexcept
		{
			ma_sound_set_positioning(&m_sound, positioning);
		}

		void Sound::set_position(const float x, const float y, const float z) noexcept
		{
			ma_sound_set_position(&m_sound, x, y, z);
		}

		void Sound::set_direction(const float x, const float y, const float z) noexcept
		{
			ma_sound_set_direction(&m_sound, x, y, z);
		}

		void Sound::set_cone(float inner_angle, const float outer_angle, const float outer_gain) noexcept
		{
			ma_sound_set_cone(&m_sound, inner_angle, outer_angle, outer_gain);
		}

		void Sound::set_velocity(const float x, const float y, const float z) noexcept
		{
			ma_sound_set_velocity(&m_sound, x, y, z);
		}

		void Sound::set_attenuation_model(ma_attenuation_model model) noexcept
		{
			ma_sound_set_attenuation_model(&m_sound, model);
		}

		void Sound::set_rolloff(const float rolloff) noexcept
		{
			ma_sound_set_rolloff(&m_sound, rolloff);
		}

		void Sound::set_min_gain(const float gain) noexcept
		{
			ma_sound_set_min_gain(&m_sound, gain);
		}

		void Sound::set_max_gain(const float gain) noexcept
		{
			ma_sound_set_max_gain(&m_sound, gain);
		}

		void Sound::set_min_distance(const float distance) noexcept
		{
			ma_sound_set_min_distance(&m_sound, distance);
		}

		void Sound::set_max_distance(const float distance) noexcept
		{
			ma_sound_set_max_distance(&m_sound, distance);
		}

		void Sound::set_doppler_factor(const float factor) noexcept
		{
		...
		bool Sound::is_looping() const noexcept
		{
			return ma_sound_is_looping(&m_sound);
		}

		bool Sound::is_finished() const noexcept
		{
