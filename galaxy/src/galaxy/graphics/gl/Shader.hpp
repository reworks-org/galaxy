...
namespace galaxy
{
	...
	template<>
	inline void Shader::set_uniform<Colour>(const std::string& name, const Colour& a)
	{
		auto vec4 = glm::vec4();

		vec4.x = std::clamp(math::normalize(a.r(), Colour::OFFSET), 0.0f, 1.0f);
		vec4.y = std::clamp(math::normalize(a.g(), Colour::OFFSET), 0.0f, 1.0f);
		vec4.z = std::clamp(math::normalize(a.b(), Colour::OFFSET), 0.0f, 1.0f);
		vec4.w = std::clamp(math::normalize(a.a(), Colour::OFFSET), 0.0f, 1.0f);

		set_uniform(name, vec4);
	}
} // namespace galaxy
