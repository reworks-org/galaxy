...
namespace galaxy
{
	namespace fs
	{
		...
		FileInfo VirtualFileSystem::find(std::string_view file)
		{
			...
			if (path.has_extension())
			{
				if (path.is_absolute())
				{
					...
					{
						info.m_code   = FileInfo::Code::FOUND;
						info.m_path   = path;
					...
					else
					{
						...
						info.m_code = FileInfo::Code::NOT_IN_VFS;
						return info;
					}
				}
				else
				{
					for (const auto& dir_entry :
						std::filesystem::recursive_directory_iterator(m_root, std::filesystem::directory_options::skip_permission_denied))
					{
			...
			else
			{
				...
				info.m_code = FileInfo::Code::NO_EXTENSION;
				return info;
			}
		}
		...
		bool VirtualFileSystem::create_file(std::string_view path, const bool binary)
		{
			...
			if (found.m_code == FileInfo::Code::NOT_FOUND)
			{
		...
		std::optional<std::string> VirtualFileSystem::open(std::string_view file)
		{
			...
			if (path.m_code == FileInfo::Code::FOUND)
			{
		...
		bool VirtualFileSystem::save(const std::string& data, std::string_view file)
		{
			...
			if (path.m_code == FileInfo::Code::FOUND || path.m_code == FileInfo::Code::NOT_FOUND)
			{
