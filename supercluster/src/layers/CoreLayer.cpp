...
{
	CoreLayer::CoreLayer()
	    : m_world {nullptr}, m_window {nullptr}, m_draw_main {true}, m_draw_demo {false}, m_draw_state_editor {false}, m_draw_json_editor {false}, m_draw_script_editor {false}, m_draw_atlas_editor {false}, m_draw_entity_editor {false}, m_draw_lua_console {false}, m_draw_gui_builder_ui {false}, m_atlas_state {-1}, m_visible_tools {true}, m_show_entity_create {false}, m_entity_debug_name {"..."}, m_active_entity {0}, m_edn_buffer {""}
	{
		set_name("sandbox_layer");
		m_window   = SL_HANDLE.window();
		m_world    = SL_HANDLE.world();
		m_renderer = SL_HANDLE.renderer();

		m_camera.create(0.0f, SL_HANDLE.window()->get_width(), SL_HANDLE.window()->get_height(), 0.0f);
		m_camera.set_speed(5.0f);

		m_default_post.m_shader.load_path("assets/shaders/render_to_texture.vs", "assets/shaders/render_to_texture.fs");
		m_renderer->add_post_effect(&m_default_post);
	...
	void CoreLayer::update(const double dt)
	{
		m_camera.update(dt);
		m_world->update(dt);
	}

	void CoreLayer::pre_render()
	{
		...
		ImGui::DockSpace(ImGui::GetID("CoreLayer_Dockspace_01"), {0.0f, 0.0f}, dockspace_flags);
	...
	{
		m_world->get_system<systems::RenderSystem>()->render(m_camera);
	...
	void CoreLayer::state_manager_ui()
	{
	...
	void CoreLayer::script_editor_ui()
	{
	...
	void CoreLayer::atlas_editor_ui()
	{
		...
		{
			SL_HANDLE.atlas()->create(*m_renderer, *SL_HANDLE.shaderbook()->get("render_to_texture"));
		...
		{
			SL_HANDLE.atlas()->update(*m_renderer, *SL_HANDLE.shaderbook()->get("render_to_texture"));
	...
	void CoreLayer::component_ui(bool enabled, std::uint64_t entity)
	{
