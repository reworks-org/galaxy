...
extern "C"
{
	 ...
	 */
	MSDFGL_EXPORT msdfgl_atlas_t msdfgl_create_atlas(msdfgl_context_t ctx, int texture_width, int padding);
	 ...
	 */
	MSDFGL_EXPORT FT_Face msdfgl_load_font(msdfgl_context_t ctx, const char* font_name);

	MSDFGL_EXPORT msdfgl_font_t _msdfgl_init_font_internal(msdfgl_context_t ctx, FT_Face* face, float range, float scale, msdfgl_atlas_t atlas);
...
#define msdfgl_generate_ascii_ext(font) msdfgl_generate_glyphs(font, 0, 255)
	 ...
	 */
	MSDFGL_EXPORT void msdfgl_render(msdfgl_font_t font, msdfgl_glyph_t* glyphs, int n, GLfloat* projection);
	 ...
	 */
	MSDFGL_EXPORT float msdfgl_printf(float x,
		float y,
		int align,
		msdfgl_font_t font,
		float size,
	 ...
	 */
	MSDFGL_EXPORT void msdfgl_geometry(float* x, float* y, msdfgl_font_t font, float size, enum msdfgl_printf_flags flags, const void* fmt, ...);

	MSDFGL_EXPORT float
	msdfgl_measure(msdfgl_font_t font, float size, unsigned int limit, GLfloat* projection, enum msdfgl_printf_flags flags, const void* fmt, ...);
	 ...
	 */
	MSDFGL_EXPORT void msdfgl_set_missing_glyph_callback(msdfgl_context_t, int (*)(msdfgl_font_t, int32_t, void*), void*);
	 ...
	 */
	MSDFGL_EXPORT void msdfgl_set_dpi(msdfgl_context_t context, float horizontal, float vertical);

	/* Plumbing commands. In case you want to build your own renderer. */
	/**
	 * Generates an orthographic projection (similar to glm's ortho).
	 */
	MSDFGL_EXPORT void _msdfgl_ortho(GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, GLfloat nearVal, GLfloat farVal, GLfloat dest[][4]);
