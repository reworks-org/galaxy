...
namespace galaxy
{
	namespace components
	{
		ParticleGenerator::ParticleGenerator() noexcept
		    : Serializable {this}, m_texture_id {""}, m_emitter_x {0.0f}, m_emitter_y {0.0f}, m_amount {0}, m_finished {true}, m_current_instance {""}, m_current_particle {0}
		{
		}
		...
		ParticleGenerator::ParticleGenerator(ParticleGenerator&& pg) noexcept
		    : Serializable {this}
		{
			this->m_texture_id          = std::move(pg.m_texture_id);
		...
		void ParticleGenerator::configure(std::string_view particle_type)
		{
			...
			{
				m_particles_instances.emplace(str, graphics::ParticleInstance {});
		...
		void ParticleGenerator::set_emitter(const float emitter_x, const float emitter_y) noexcept
		{
			m_emitter_x = emitter_x;
			m_emitter_y = emitter_y;
		}
		...
		void ParticleGenerator::clear()
		{
			...
			m_finished         = true;
			m_current_instance = "";
			m_current_particle = 0;
		}

		graphics::ParticleInstance* const ParticleGenerator::get_instance() noexcept
		{
		...
		nlohmann::json ParticleGenerator::serialize()
		{
			...
			json["particle-sheet"]     = m_texture_id;
			json["emitter-x"]          = m_emitter_x;
			json["emitter-y"]          = m_emitter_y;
			json["defines"]            = nlohmann::json::object();
			json["types-to-configure"] = nlohmann::json::array();

			for (const auto& [name, rect] : m_texture_regions)
			{
				json["defines"][name]["x"] = rect.m_x;
				json["defines"][name]["y"] = rect.m_y;
				json["defines"][name]["w"] = rect.m_width;
				json["defines"][name]["h"] = rect.m_height;
			}

			for (const auto& [name, particle] : m_particles_instances)
			{
				json["types-to-configure"].push_back(name);
			}
