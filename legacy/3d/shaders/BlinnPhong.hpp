...
namespace galaxy
{
	namespace shaders
	{
		...
		///
		/// Blinn-Phong fragment shader part 1.
		///
		inline constexpr const char* const blinnphong_frag_1 = R"(
            #version 450 core
		...
		///
		/// Blinn-Phong fragment shader part 2.
		///
		inline constexpr const char* const blinnphong_frag_2 = R"(
            const vec2 g_poisson_25[25] = vec2[](
            ...
            const vec2 g_poisson_32[32] = vec2[](
            ...
            const vec2 g_poisson_64[64] = vec2[](
            ...
            const vec2 g_poisson_100[100] = vec2[](
            ...
            const vec2 g_poisson_128[128] = vec2[](
		...
		///
		/// Blinn-Phong fragment shader part 3.
		///
		inline constexpr const char* const blinnphong_frag_3 = R"(
            // TEMP
            float u_BlockerSearchScale = 1.0;
            float u_LightSize = 1.0;
            int u_BlockerSearchSamples = 128;
            int u_PCFSamples = 128;
            float u_LightBias = 0.008;
            // TEMP

            // Blinn-Phong light calcs.
            ...
            float do_pcss_shadowing(vec3 p);
            ...
            void main()
            {
	            ...
	            vec3 result = calc_directional(b_dir_light, normals, view_dir, diff_tex, spec_map, shininess);

                // Shadow mapping.
                float shadow = do_pcss_shadowing(frag_pos);
                result *= shadow;
            ...
            vec3 calc_directional(DirLight light, vec3 normal, vec3 view_dir, vec3 diff_tex, vec3 spec_map, float shininess)
            {
                ...
                float fresnel = dot(halfway_dir, view_dir);
                fresnel = max(fresnel, 0.0);
                fresnel = pow(1.0 - fresnel, 5);
            ...
            float search_region_radius_uv(float z)
            {
                return u_BlockerSearchScale * u_LightSize * (z - b_dir_light.near) / z;
            }
            ...
            float project_to_light_uv(float penumbra_radius, float z)
            {
                return penumbra_radius * u_LightSize * b_dir_light.near / z;
            }

            float z_clip_to_eye(float z)
            {
                return b_dir_light.far * b_dir_light.near / (b_dir_light.far - z * (b_dir_light.far - b_dir_light.near));
            }
            ...
            float do_pcss_shadowing(vec3 p)
            {
                ...
                vec4 light_space_pos = b_dir_light.proj * b_dir_light.view * vec4(p, 1.0);
                ...
                vec4 pos_vs = b_dir_light.view * vec4(p, 1.0);
                pos_vs.xyz /= pos_vs.w;
