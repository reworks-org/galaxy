...
namespace galaxy
{
	namespace systems
	{
		...
		{
			scene->m_world.operate<components::Transform2D, components::Renderable>(std::execution::par,
				[&](const ecs::Entity entity, components::Transform2D* transform, components::Renderable* renderable) {
							...
							case graphics::Renderables::BATCHED:
								{
									...
									renderable->update_aabb({transform->get_pos().x,
										transform->get_pos().y,
										batchsprite->get_region().m_width,
										batchsprite->get_region().m_height});
								}
								break;
