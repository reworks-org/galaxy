...
namespace galaxy
{
	namespace ui
	{
		    ...
		    : m_text {text}, m_speaker {speaker}
		{
			...
			m_speaker_spr.create(ORTHO_FAR_24BIT);
		}
		...
		void Textbox::create(std::string_view box, std::string_view indicator, std::string_view font, float border_width)
		{
			...
			m_box.create(box, ORTHO_FAR_24BIT - 1);
			m_indicator.create(indicator, ORTHO_FAR_24BIT);

			m_bounds.m_width  = m_box.get_region().m_width;
			m_bounds.m_height = m_box.get_region().m_height;

			m_text.load(font, m_theme->m_font_col);
			m_text.create("", ORTHO_FAR_24BIT);
			...
			m_indicator_timer.set([&]()
			{
				...
				{
					m_draw_lower = true;
					m_indicator_transform.set_pos(m_ind_x, m_ind_y + (m_indicator.get_region().m_height / 4.0f));
				}
			},
			800);
			...
			m_theme->m_sb.add(&m_box, &m_box_transform);
			m_theme->m_sb.add(&m_indicator, &m_indicator_transform);
		...
		void Textbox::render()
		{
			if (!(m_messages_index >= m_messages.size()))
			{
				...
				if (!cur_msg.m_speaker.empty())
				{
					...
					RENDERER_2D().draw_sprite(&cur_msg.m_speaker_spr, &cur_msg.m_speaker_tf);
				}
			}
		...
		void Textbox::set_pos(const float x, const float y) noexcept
		{
			...
			m_ind_x = ((m_bounds.m_x + m_bounds.m_width) - m_indicator.get_region().m_width) - m_border_width;
			m_ind_y = ((m_bounds.m_y + m_bounds.m_height) - m_indicator.get_region().m_height) - m_border_width;
		}
		...
		nlohmann::json Textbox::serialize()
		{
			...
			json["box"]          = m_box.get_key();
			json["indicator"]    = m_indicator.get_key();
