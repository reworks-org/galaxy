...
std::size_t read_ogg_callback(void* dest, std::size_t size_a, std::size_t size_b, void* handle)
{
	...
	auto* stream   = reinterpret_cast<galaxy::audio::BufferStream*>(handle);
	...
	if (!data->m_file_handle.is_open())
	{
		...
		if (!data->m_file_handle.is_open())
		{
			GALAXY_LOG(GALAXY_ERROR, "Could not re-open streaming file: {0}.", data->m_file_path.string());
			return 0;
		}
	}
	...
	if (!data->m_file_handle.read(&buff[0], length))
	{
		...
		{
			GALAXY_LOG(GALAXY_ERROR, "OGG stream has fail bit set: {0}.", data->m_file_path.string());
			data->m_file_handle.clear();
		...
		{
			GALAXY_LOG(GALAXY_ERROR, "OGG stream has bad bit set: {0}.", data->m_file_path.string());
			data->m_file_handle.clear();
...
namespace galaxy
{
	namespace audio
	{
		...
		bool BufferStream::internal_load(std::string_view file)
		{
			...
			{
				GALAXY_LOG(GALAXY_ERROR, "File must be ogg vorbis and have extension of .ogg: {0}.", m_data.m_file_path.string());
				result = false;
			}
			else
			{
				...
				{
					GALAXY_LOG(GALAXY_ERROR, "Failed to open file: {0}.", m_data.m_file_path.string());
					result = false;
				}
				else
				{
					...
					{
						GALAXY_LOG(GALAXY_ERROR, "ov_open_callbacks failed for: {0}.", m_data.m_file_path.string());
						result = false;
					}
					else
					{
						...
						{
							GALAXY_LOG(GALAXY_ERROR, "Already reached EOF without loading data: {0}.", m_data.m_file_path.string());
							result = false;
						}
						else if (m_data.m_file_handle.fail())
						{
							GALAXY_LOG(GALAXY_ERROR, "fail bit set: {0}.", m_data.m_file_path.string());
							result = false;
						}
						else if (!m_data.m_file_handle)
						{
							GALAXY_LOG(GALAXY_ERROR, "file is false: {0}.", m_data.m_file_path.string());
							result = false;
						}
						else
						{
							...
							for (std::uint8_t i = 0; i < BUFFER_COUNT; ++i)
							{
								...
								while (processed < BUFFER_SIZE)
								{
									...
									{
										GALAXY_LOG(GALAXY_ERROR, "OV_HOLE found in initial read of buffer: {0}.", m_data.m_file_path.string());
										result = false;
									...
									{
										GALAXY_LOG(GALAXY_ERROR, "OV_EBADLINK found in initial read of buffer: {0}.", m_data.m_file_path.string());
										result = false;
									...
									{
										GALAXY_LOG(GALAXY_ERROR, "OV_EINVAL found in initial read of buffer: {0}.", m_data.m_file_path.string());
										result = false;
									...
									{
										GALAXY_LOG(GALAXY_ERROR, "EOF found in initial read of buffer: {0}.", m_data.m_file_path.string());
										result = false;
