...
namespace rbp 
{
	...
	re::Rect<int> MaxRectsBinPack::Insert(int width, int height, FreeRectChoiceHeuristic method)
	{
		re::Rect<int> newNode;
	...
	void MaxRectsBinPack::Insert(std::vector<re::Vector2<int>> &rects, std::vector<re::Rect<int>> &dst, FreeRectChoiceHeuristic method)
	{
		dst.clear();
		...
		while (rects.size() > 0)
		{
			...
			int bestRectIndex = -1;
			re::Rect<int> bestNode;

			for (size_t i = 0; i < rects.size(); ++i)
			{
				int score1;
				int score2;
				re::Rect<int> newNode = ScoreRect(rects[i].x, rects[i].y, method, score1, score2);
	...
	void MaxRectsBinPack::PlaceRect(const re::Rect<int> &node)
	{
		size_t numRectanglesToProcess = freeRectangles.size();
		for (size_t i = 0; i < numRectanglesToProcess; ++i)
		{
	...
	re::Rect<int> MaxRectsBinPack::ScoreRect(int width, int height, FreeRectChoiceHeuristic method, int &score1, int &score2) const
	{
		re::Rect<int> newNode;
	...
	re::Rect<int> MaxRectsBinPack::FindPositionForNewNodeBottomLeft(int width, int height, int &bestY, int &bestX) const
	{
		re::Rect<int> bestNode;
		memset(&bestNode, 0, sizeof(re::Rect<int>));
	...
	re::Rect<int> MaxRectsBinPack::FindPositionForNewNodeBestShortSideFit(int width, int height,
	...
	re::Rect<int> MaxRectsBinPack::FindPositionForNewNodeBestLongSideFit(int width, int height,
		int &bestShortSideFit, int &bestLongSideFit) const
	{
		re::Rect<int> bestNode;
		memset(&bestNode, 0, sizeof(re::Rect<int>));
	...
	re::Rect<int> MaxRectsBinPack::FindPositionForNewNodeBestAreaFit(int width, int height,
	...
	re::Rect<int> MaxRectsBinPack::FindPositionForNewNodeContactPoint(int width, int height, int &bestContactScore) const
	{
		re::Rect<int> bestNode;
		memset(&bestNode, 0, sizeof(re::Rect<int>));
	...
	bool MaxRectsBinPack::SplitFreeNode(re::Rect<int> freeNode, const re::Rect<int> &usedNode)
	{
