      ...
      _height(0)
{
	...
	glAttachShader(_programId, v_id);
	glAttachShader(_programId, f_id);
...
void FTLabel::recalculateVertices(const char* text, float x, float y, int width, int height)
{
	...
	std::vector<std::string> words = splitText(_text.c_str());
...
void FTLabel::setText(std::string text)
{
	setText(text.c_str());
}
