...
#define NOTIFY_TOAST_FLAGS                                                                                                   \
	ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoNav | \
		ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoSavedSettings
...
private:
	...
	inline auto set_title(const char* format, va_list args)
	{
		vsnprintf(this->title, sizeof(this->title), format, args);
	}

	inline auto set_content(const char* format, va_list args)
	{
		vsnprintf(this->content, sizeof(this->content), format, args);
	}

public:
	inline auto set_title(const char* format, ...) -> void
	{
		NOTIFY_FORMAT(this->set_title, format);
	}

	inline auto set_content(const char* format, ...) -> void
	{
		NOTIFY_FORMAT(this->set_content, format);
	}

	inline auto set_type(const ImGuiToastType& type) -> void
	{
...
public:
	...
	inline auto get_title() -> const char*
	{
		return this->title;
	};

	inline auto get_default_title() -> const char*
	{
	...
	inline auto get_type() -> const ImGuiToastType&
	{
		return this->type;
	};
	...
	inline auto get_content() -> const char*
	{
		return this->content;
	};
