...
namespace galaxy
{
	namespace state
	{
		MapLayer::MapLayer(std::string_view name, Scene* scene) noexcept
			: Layer {name, scene}
		{
		}

		MapLayer::~MapLayer() noexcept
		{
		}

		void MapLayer::on_push()
		{
		}

		void MapLayer::on_pop()
		{
			auto& ae = core::ServiceLocator<audio::AudioEngine>::ref();
			ae.stop_all();
		}

		void MapLayer::events()
		{
		...
		nlohmann::json MapLayer::serialize()
		{
			nlohmann::json json = "{}"_json;
			json["name"]        = m_name;
			json["type"]        = "Map"; // Used by Layers stack.
			json["world"]       = m_world.serialize();

			return json;
		}

		void MapLayer::deserialize(const nlohmann::json& json)
		{
			m_name = json.at("name");
