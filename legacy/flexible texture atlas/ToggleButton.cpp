...
namespace galaxy
{
	namespace ui
	{
		...
		{
			m_batched.create(on);
			m_regions[0] = SL_HANDLE.atlas()->get_tex_info(on).m_region;
			m_regions[1] = SL_HANDLE.atlas()->get_tex_info(off).m_region;
			m_regions[2] = SL_HANDLE.atlas()->get_tex_info(on_hover).m_region;
			m_regions[3] = SL_HANDLE.atlas()->get_tex_info(off_hover).m_region;
