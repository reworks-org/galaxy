...
namespace galaxy
{
	namespace ui
	{
		...
		void Button::create(std::string_view def, std::string_view pressed, std::string_view hover)
		{
			...
			m_regions[0] = SL_HANDLE.atlas()->get_tex_info(def).m_region;
			m_regions[1] = SL_HANDLE.atlas()->get_tex_info(pressed).m_region;
			m_regions[2] = SL_HANDLE.atlas()->get_tex_info(hover).m_region;
